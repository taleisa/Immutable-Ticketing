...
class EventHost:
    ...
    GEA = "0xEaeF9E27B9b75E6aa16CAEc72a8f577e13E9CbDD"      # Account[3] address GEA
    # connecting the blockchain network in ganache
    w3 = Web3(Web3.HTTPProvider('HTTP://127.0.0.1:7545'))
    #retrieving the generated ABI and Byte code from the 
    truffleFile = json.load(open('static/marketplace/TicketNFT.json')) # truffle generated file
    ...
    def grantMinterRole(self, contract):  
        # Send transaction
        send_tx = contract.functions.grantRole(Web3.solidityKeccak(["bytes32"],[str.encode("MINTER_ROLE")]) ,EventHost.GEA).transact({"from": self.event_host_address})
    ...
    def grantGEARole(self, contract):
        # Send transaction
        send_tx = contract.functions.grantRole(Web3.solidityKeccak(["bytes32"],[str.encode("GEA_ACCOUNTS")]) ,EventHost.GEA).transact({"from": self.event_host_address})
    ...
    def grantCustomerRole(self, cust_add, contract):
        # Send transaction
        send_tx = contract.functions.grantRole(Web3.solidityKeccak(["bytes32"],[str.encode("VERIFIED_ACCOUNTS")]) ,cust_add).transact({"from": self.event_host_address})
